...
let web3 = new Web3("wss://wsapi.fantom.network/");
let web4 = new Web3("https://rpc.ftm.tools/");
        ...
        if (msg.text) {
            ...
            if (tokenized[0] == '/add') {
                if (tokenized.length != 3) {
                    bot.sendMessage(chatId, "invalid command i.e. /add ElonMusk 0x4d9361a86d038c8ada3db2457608e2275b3e08d4")
                } else if (myCache.get("targetAddress").filter(x => x.address == tokenized[2].length != 0)) {
            ...
            if (tokenized[0] == '/delete') {
                if (tokenized.length != 3) {
                    bot.sendMessage(chatId, "invalid command i.e. /delete 0x4d9361a86d038c8ada3db2457608e2275b3e08d4")
                } else if (myCache.get("targetAddress").filter(x => x.address == tokenized[1])) {
                    bot.sendMessage(chatId, "address already in database")
                } else if (!web3.utils.isAddress(tokenized[2])) {
                    bot.sendMessage(chatId, "invalid address")
                } else {
